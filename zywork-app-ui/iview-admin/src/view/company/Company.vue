<template>
  <div>
          ...
          <Row>
            <i-col span="11">
              <FormItem prop="jurisdictionTypeMin">
                <InputNumber
                  v-model="searchForm.jurisdictionTypeMin"
                  placeholder="请输入开始辖区类别"
                  style="width: 100%;"
                />
              </FormItem>
            </i-col>
            ...
            <i-col span="11">
              <FormItem prop="jurisdictionTypeMax">
                <InputNumber
                  v-model="searchForm.jurisdictionTypeMax"
                  placeholder="请输入结束辖区类别"
                  style="width: 100%;"
                />
              </FormItem>
            </i-col>
    ...
    >
      ...
      <p>
        辖区类别:
        <span v-text="form.jurisdictionType"></span>
      </p>
      <p>
        市区:
      ...
      <p>
        企业类型:
        <span v-text="form.compType"></span>
      </p>
      <p>
        行业分类:
        <span v-text="form.industryType"></span>
      </p>
      <p>
        企业名称:
      ...
      <p>
        法人:
        <span v-text="form.legalPerson"></span>
      </p>
      <p>
        法人电话:
        <span v-text="form.legalPersonPhone"></span>
      </p>
      <p>
        注册地区:
        <span v-text="form.regAddress"></span>
      </p>
      <p>
        负责人:
        <span v-text="form.responsible"></span>
      </p>
      <p>
        负责人电话:
        <span v-text="form.responsiblePhone"></span>
      </p>
      <p>
        单位电话:
      ...
      <p>
        注册资本:
        <span v-text="form.regCapital/100"></span>
      </p>
      <p>
        经营范围:
      ...
      <p>
        可承担业务:
        <span v-text="form.affordableBusiness"></span>
      </p>
      <p>
        源地址：
    ...
    >
      <Form ref="pythonForm" :model="python" :label-width="80" :rules="validateRules">
        <FormItem label="数据来源" prop="sourse">
          <a
            href="http://jxsggzy.cn/web/tradeSubject_list.html"
            target="_blank"
          >http://jxsggzy.cn/web/tradeSubject_list.html</a>
        </FormItem>
        <FormItem label="爬取说明" prop="desc">爬取到相关的企业信息的同时，会爬取该企业相关的数据，如：企业人员、企业业绩、企业资质、企业建造师</FormItem>
...
import { projectCity, region, compType, industryType } from '@/api/select'
  ...
  methods: {
    ...
    setAddress() {
      ...
      if (this.compAddr.length > 0) {
        this.form.compAddr =
          this.compAddr[0] + '/' + this.compAddr[1] + '/' + this.compAddr[2]
      }
    },
    setPrice(type) {
      if (type === 0) {
        ...
        if (
          this.form.regCapitalDisplay !== null &&
          this.form.regCapitalDisplay !== 0
        ) {
    ...
    initIndustryTypeList(val) {
      ...
      if (
        this.industryTypeList !== undefined &&
    ...
    crawlData() {
      ...
      if (!this.python.pageSize) {
        this.$Message.error('请输入个数')
        return
      }
      if (!this.python.compType) {
        this.$Message.error('请选择企业类型')
        return
      }
      if (!this.python.industryType) {
        this.$Message.error('请选择行业类型')
        return
      }
      if (!this.python.compName) {
