    ...
    >
      <Form ref="addForm" :model="form" :label-width="100" :rules="validateRules">
        ...
        <FormItem label="中文说明" prop="description">
          <Input v-model="form.description" type="textarea" :autosize="descriptionAutoSize" placeholder="请输入中文说明"/>
        </FormItem>
  ...
  methods: {
    ...
    userOpt(itemName, row) {
      if (itemName === 'showEdit') {
        ...
        this.form.value = JSON.parse(row.value)
        this.form.comment = JSON.parse(row.comment)
