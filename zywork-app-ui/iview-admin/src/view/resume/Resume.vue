			...
			<Form ref="addForm" :model="form" :label-width="80" :rules="validateRules">
				...
				<FormItem label="学历" prop="education">
					<Select v-model="form.education" placeholder="请选择学历" clearable filterable>
						<i-option v-for="item in educationList" :value="item.value" :key="item.key">{{item.label}}</i-option>
					</Select>
				</FormItem>
				<FormItem label="要求薪资/月" prop="salary">
					<Select v-model="form.salary" placeholder="请选择要求薪资/月" clearable filterable>
						<i-option v-for="item in salaryRangeList" :value="item.value" :key="item.key">{{item.label}}</i-option>
					</Select>
				</FormItem>
				...
				<FormItem label="个人简介" prop="intorduce">
					<Input v-model="form.intorduce" type="textarea" :autosize="descriptionAutoSize" placeholder="请输入个人简介" />
				</FormItem>
