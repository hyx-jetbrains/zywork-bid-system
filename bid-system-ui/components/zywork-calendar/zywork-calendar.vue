		...
		<view class="uni-calendar__box">
			<view class="uni-calendar__wrapper">
				<view class="uni-calenda__content">
					...
					<zywork-calendar-item v-if="slide === 'none'" :canlender="canlender" :lunar="lunar" @selectDays="selectDays"></zywork-calendar-item>
					<swiper v-else class="uni-calendar__body" :style="{ height: domHeihgt + 'rpx' }" :current="currentIndex" circular :vertical="slide === 'vertical' ? true : false" skip-hidden-item-layout :duration="duration" @animationfinish="animationfinish" @change="change">
						<swiper-item v-for="(item, itemindx) in swiperData" :key="itemindx">
							<view :class="elClass">
								<zywork-calendar-item :canlender="item" :lunar="lunar" @selectDays="selectDays"></zywork-calendar-item>
							</view>
						</swiper-item>
					</swiper>
				</view>
...
<script>
	...
	import zyworkCalendarItem from './zywork-calendar-item';
	export default {
		name: 'zywork-calendar',
		components: {
			zyworkCalendarItem
		},
		props: {
		...
		methods: {
			...
			backtoday() {
				...
				this.indexTimer = setTimeout(() => {
					...
					this.isSilde = true;
					if (this.slide === 'none') {
						this.getMonthAll(0, this.getDate(new Date()));
					} else {
						this.getMonthAll(1, this.getDate(new Date()));
					}
					this.hold = false;
				}, 200);
			},
