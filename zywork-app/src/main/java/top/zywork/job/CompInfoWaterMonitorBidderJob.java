...
public class CompInfoWaterMonitorBidderJob implements Job {

    private static final Logger logger = LoggerFactory.getLogger(CompInfoWaterMonitorBidderJob.class);
    ...
    public void execute(JobExecutionContext jobExecutionContext) throws JobExecutionException {
        ...
        String path = filesPath + PythonConstants.PAGE_NO_FILE_NAME_COMP_WATER_MONITOR_BIDDER;
        String pageNo = IOUtils.getText(path).replaceAll("\r\n", "");
        try {
            companyPythonService.getCompanyInfo(PythonConstants.TYPE_BIDDER, PythonConstants.COMP_TYPE_WATER_MONITOR_BIDDER, pageNo, PythonConstants.DEFAULT_PAGE_SIXE, PythonConstants.DEFAULT_IS_UPDATE_FLAG);
            int tempPageNo = Integer.valueOf(pageNo);
            tempPageNo++;
            IOUtils.writeText(tempPageNo + "", path);
        } catch (Exception e) {
            logger.error("CompInfoWaterMonitorBidderJob Error:pageNo:{}", pageNo);
        }
